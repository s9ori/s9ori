...
class LiveSiteControlProvider extends ServiceProvider {
	 ...
	 * right data.
	 */
	public function milestone_publish_later_nux_api() {
		$domain = defined( 'GD_TEMP_DOMAIN' ) ? GD_TEMP_DOMAIN : Helper::domain();
		$url    = Helper::wpnux_api_base() . '/milestones/site-publish-later?domain=' . $domain;

		$remote_post_raw = $this->perform_remote_api_post( $url, Helper::get_default_nux_api_request_body() );
		$remote_post     = $this->format_remote_post_response( $remote_post_raw );
			...
			function () {
				...
				register_rest_route(
					...
					array(
						...
						'show_in_index'       => false,
						'callback'            => array( $this, 'milestone_publish_later_nux_api' ),
					)
				);
			}
		);
