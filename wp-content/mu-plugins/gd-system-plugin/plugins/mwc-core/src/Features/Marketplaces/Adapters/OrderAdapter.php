...
class OrderAdapter implements DataSourceAdapterContract
{
    ...
    public function convertFromSource() : Order
    {
        ...
        // Customer note.
        if ($customerNote = ArrayHelper::get($this->source, 'customer_notes')) {
            $order->setCustomerNote((string) $customerNote);
        }
        ...
        if ($marketplacesInternalOrderNumber = ArrayHelper::get($this->source, 'sb_order_seq')) {
            $order->setMarketplacesInternalOrderNumber($marketplacesInternalOrderNumber);
        }
        if ($marketplacesDisplayOrderNumber = ArrayHelper::get($this->source, 'display_ref')) {
            $order->setMarketplacesDisplayOrderNumber($marketplacesDisplayOrderNumber);
        }
        if ($marketplacesChannelOrderReference = ArrayHelper::get($this->source, 'order_id')) {
            $order->setMarketplacesChannelOrderReference($marketplacesChannelOrderReference);
        }
        if ($marketplacesChannelUuid = ArrayHelper::get($this->source, 'channel_uuid')) {
            $order->setMarketplacesChannelUuid($marketplacesChannelUuid);
        }
        if ($marketplacesChannelName = ArrayHelper::get($this->source, 'channel_name')) {
            $order->setMarketplacesChannelName($marketplacesChannelName);
        }
        if ($marketplacesChannelType = ArrayHelper::get($this->source, 'channel_type_display_name')) {
            $order->setMarketplacesChannelType(strtolower($marketplacesChannelType));
        }
    ...
    protected function convertFulfillmentStatusFromSource() : FulfillmentStatusContract
    {
        ...
        $statusName = ArrayHelper::get($this->source, 'shipment_status', '');
        $statusClass = ArrayHelper::get($nativeStatuses, $statusName ?: '');
