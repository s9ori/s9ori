...
class MarketplacesPage extends AbstractAdminPage implements ComponentContract
{
    ...
    protected function renderManageConnectedSalesChannels() : void
    {
        foreach (ChannelRepository::getConnected(false) as $channel) {?>
            <p><a href="<?php echo esc_url(Marketplaces::getMarketplacesUrl('/channels/'.$channel->getId())); ?>" target="_blank"><span style="margin-right: 8px;"><?php echo ChannelRepository::getIcon($channel->getType()); ?></span><?php echo ChannelRepository::getLabel($channel->getType() ?: '');
            $this->renderNewWindowIcon(); ?></a></p>
        <?php } ?>
        <?php $this->renderAddSalesChannelButton(); ?>
        <p><a href="<?php echo esc_url(Marketplaces::getMarketplacesUrl('/settings/templates')); ?>" target="_blank"><?php esc_html_e('Manage Listing Templates', 'mwc-core');
        $this->renderNewWindowIcon(); ?></a></p>
        <?php
    }
    ...
    {
        $buttonBehavior = 'onclick="window.open(\''.esc_url(Marketplaces::getMarketplacesUrl('/settings/channels')).'\', \'_blank\');"';
