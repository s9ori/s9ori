...
class MarketplacesMetabox extends AbstractPostMetabox implements ComponentContract
{
    ...
    protected function renderChannelPanel(?Product $product, ?WP_Post $post, string $channelTypeName, ?Listing $listing)
    {
        ...
        <div id="<?php echo esc_attr("gd-marketplaces-{$channelTypeSlug}"); ?>" class="panel woocommerce_options_panel">
            <?php
                if (empty($channel)) {
                    $this->renderChannelNotConnected($product, $post, $channelTypeName);
     ...
     * @return void
     */
    protected function renderChannelNotConnected(?Product $product, ?WP_Post $post, string $channelTypeName) : void
    {
        ?>
        <p>
        ...
        <?php try { ?>
            <p>
                <a href="<?php echo esc_url(Marketplaces::getMarketplacesUrl('/settings/channels')); ?>" target="_blank" class="button button-primary"><?php esc_html_e('Connect Sales Channel', 'mwc-core'); ?></a>
            </p>
            ...
            <div class="gd-marketplaces-product-requirements">
                <p>
                    ...
                    <?php printf(
                        ...
                        esc_html__('A draft listing will automatically created once the product meets the requirements below. %1$sLearn more about product sync%2$s', 'mwc-core'),
    ...
    public function renderDraftListingCreated(?Product $product, ?WP_Post $post, Listing $listing) : void
    {
        ...
        <p>
            <a href="<?php echo esc_url($this->getManageListingUrl($listing->getLink())); ?>" target="_blank" class="button"><?php esc_html_e('View Draft Listing', 'mwc-core'); ?></a>
        </p>
        <?php
    }
