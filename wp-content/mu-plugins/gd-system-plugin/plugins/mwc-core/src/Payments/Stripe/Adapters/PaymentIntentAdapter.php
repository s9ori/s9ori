...
class PaymentIntentAdapter implements DataSourceAdapterContract
{
    ...
    public function convertFromSource() : array
    {
        ...
        ArrayHelper::set($data, 'payment_method_types', $this->getValidPaymentMethodTypes());
    ...
    /**
     * Gets the payment method types that are valid for the source payment intent.
     *
     * Certain payment method types can't be used if the intent is set up for future usage.
     *
     * @return string[]
     */
    protected function getValidPaymentMethodTypes() : array
    {
        $paymentMethodTypes = Configuration::get('payments.stripe.activePaymentMethods', []);

        if (! $this->source->getSetupFutureUsage()) {
            return $paymentMethodTypes;
        }

        return array_values(array_intersect($paymentMethodTypes, Stripe::getReusablePaymentMethodTypes()));
    }
