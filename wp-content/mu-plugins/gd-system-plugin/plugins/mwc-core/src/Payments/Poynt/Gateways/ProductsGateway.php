...
class ProductsGateway extends AbstractGateway
{
    ...
    {
        $products = $this->doAdaptedRequest($this, $this->getNewProductListRequestAdapter($this));

        return empty($selection) ? $products : array_filter($products, static function ($product) use ($selection) {
