...
class CatalogsGateway extends AbstractGateway
{
    ...
    {
        $catalogs = $this->doAdaptedRequest($this, new CatalogListRequestAdapter($this));

        return empty($selection) ? $catalogs : array_filter($catalogs, static function ($catalog) use ($selection) {
