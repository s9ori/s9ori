...
class LocalDelivery extends WC_Shipping_Method
{
     ...
     *
     * @param array<mixed>|mixed $package package information
     * @return void
    ...
    public function calculate_shipping($package = [])
    {
        ...
        if (WooCommerceRepository::isWooCommerceActive()) {
            $total = TypeHelper::float(WooCommerceRepository::getCartInstance()->get_displayed_subtotal(), 0.0);
            $total = $this->formatPrice($total);

            $rate['cost'] = $this->determineShippingRate($total, TypeHelper::array($package, []));
     ...
     * @return string|null
     */
    protected function executeFeeShortcode($sum, array $details = []) : ?string
    {
        // Expand shortcodes.
     ...
     * @throws Exception
     */
    protected function sanitizeSum(string $sum = '') : ?string
    {
        if (! WooCommerceRepository::isWooCommerceActive()) {
            return $sum;
        }
     ...
     * @return string
     */
    public function fee($attributes) : string
    {
        $attributes = shortcode_atts(['percent' => '', 'min_fee' => '', 'max_fee' => ''], $attributes, 'fee');
        $percent = TypeHelper::float(ArrayHelper::get($attributes, 'percent'), 0.0);
        $minFee = TypeHelper::float(ArrayHelper::get($attributes, 'min_fee'), 0.0);
        $maxFee = TypeHelper::float(ArrayHelper::get($attributes, 'max_fee'), 0.0);
        $calculatedFee = 0;

        if ($percent > 0) {
            $calculatedFee = TypeHelper::float($this->feeCost, 0.0) * ($percent / 100);
        }
        ...
        return (string) $calculatedFee;
    }

    /**
     * See if free shipping is available based on the package and cart.
     *
     * @param array<mixed>|mixed $package shipping package
     * @return bool
    ...
    public function is_available($package) : bool
    {
        ...
        if (WooCommerceRepository::getCartInstance()->display_prices_including_tax()) {
            $total = $total - TypeHelper::float(WooCommerceRepository::getCartInstance()->get_discount_tax(), 0.0);
        }

        if ('no' === $ignoreDiscounts) {
            $total = $total - TypeHelper::float(WooCommerceRepository::getCartInstance()->get_discount_total(), 0.0);
        }
        ...
        return (bool) apply_filters("woocommerce_shipping_{$this->id}_is_available", $isAvailable, $package, $this);
    }
