...
class StripeGateway extends AbstractPaymentGateway
{
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['payment_methods_title'] = [
            'title' => esc_html__('Payment Methods', 'mwc-core'),
            'type'  => 'title',
            'class' => 'mwc-stripe-payment_methods_title',
        ];

        $this->form_fields['active_payment_methods'] = [
            'title'    => esc_html__('Active Payment Methods', 'mwc-core'),
            'type'     => 'multiselect',
            'desc_tip' => esc_html__('By default, all active Stripe payment methods are enabled on your store. Credit and debit card payments will remain active, but you can disable alternative methods you don’t wish to use.', 'mwc-core'),
            'default'  => [],
    ...
    /**
     * Gets active payment methods for the connected account.
     *
     * @return string[]
     * @throws ApiErrorException
     */
    ...
    {
        if (! $this->isStripeSettingsPage()) {
            return [];
        }

        $knownCapabilities = $this->getKnownPaymentMethods();

        if ((! $account = Stripe::getAccount()) || (! $rawCapabilities = $account->getCapabilities())) {
        ...
        foreach ($rawCapabilities as $rawCapability) {
            $cleanCapability = (string) str_replace(['_payments', '_ach'], '', $rawCapability);
            if ($label = (string) ArrayHelper::get($knownCapabilities, $cleanCapability)) {
                $capabilities[$cleanCapability] = $label;
            } else {
                $capabilities[$cleanCapability] = ucwords(str_replace('_', ' ', $cleanCapability));
            }
        }
    ...
    public function generate_connect_html(string $key, array $data) : string
    {
        ...
        <tr valign="top">
            ...
            <td class="forminp">
                <a href="<?php echo esc_attr($data['action']); ?>"
    ...
    protected function updateConfigurationFromSettings($configurations = null) : void
    {
        ...
        Configuration::set("payments.{$this->providerName}.activePaymentMethods", $this->get_option('active_payment_methods', []));
    }
}
