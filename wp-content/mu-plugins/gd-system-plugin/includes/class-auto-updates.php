...
final class Auto_Updates {
	...
	private function set_auto_update_for_customer() {
		...
		if ( ! wp_next_scheduled( 'mwp_schedule_plugins_and_themes' ) && ! get_option( 'mwp_previous_plugins_state' ) && ! get_option( 'mwp_auto_updates_status' ) ) {

			//Add default option based on creation time
			if ( defined( 'GD_SITE_CREATED' ) && ( GD_SITE_CREATED > 1653868800  ) ) {

				add_option( 'mwp_auto_updates_status', 'enabled' );
				add_action( 'init', [ $this, 'migrate_old_plugins_and_themes' ] );

			} elseif( Plugin::is_site_in_percentage( 100, null, 1653868800 ) ) { // 05.30.2022 00:00:00 GMT + 100% before that date

				wp_schedule_single_event( time() + ( GD_SITE_CREATED / 1000 ) % 4 * 24 * 3600, 'mwp_schedule_plugins_and_themes' );
